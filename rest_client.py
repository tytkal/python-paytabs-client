...
class RestClient:
    ...
    def get(self, append_url='', params={}, is_auth=False):
        if is_auth:
            responce = requests.get(self.host + self.resourse + append_url, headers=self.headers, params=params,
    ...
    def post(self, params={}, append_url='', is_auth=False):
        if is_auth:
            responce = requests.post(self.host + self.resourse + append_url, data=params, headers=self.headers,
                                          auth=(self.username, self.password))
        else:
            responce = requests.post(self.host + self.resourse + append_url, data=params, headers=self.headers)
        return responce
    ...
    def put(self, params={}, append_url='', is_auth=False):
        if is_auth:
            responce = requests.put(self.host + self.resourse + append_url, data=params, headers=self.headers,
                                         auth=(self.username, self.password))
        else:
            responce = requests.put(self.host + self.resourse + append_url, data=params, headers=self.headers)
        return responce
    ...
    def delete(self, append_url='', params={}, is_auth=False):
        if is_auth:
            responce = requests.delete(self.host + self.resourse + append_url, params=params, headers=self.headers,
                                            auth=(self.username, self.password))
        else:
            responce = requests.delete(self.host + self.resourse + append_url, params=params, headers=self.headers)
        return responce
